/**
 * Países (ISO 3166-1 alfa-2) con su nombre en español, ordenados alfabéticamente.
 * Código puro compartido por formularios de clientes, proveedores y empresa.
 */
...
/** Estados miembros de la UE (ISO alfa-2). Grecia usa EL en VIES. */
...
export function isCountryCode(value: string | null | undefined) {
  return byCode.has(normalizeCountryCode(value));
}

/** Nombre en español del país (FR → Francia). Devuelve el código si no se conoce. */
export function countryName(value: string | null | undefined) {
  const code = normalizeCountryCode(value);
  return byCode.get(code)?.name ?? code;
}

export function isEuCountry(value: string | null | undefined) {
  return EU_MEMBER_CODES.has(normalizeCountryCode(value));
}

/** Prefijo de país que usa VIES (Grecia = EL). */
export function viesCountryPrefix(value: string | null | undefined) {
  const code = normalizeCountryCode(value);
  return code === "GR" ? "EL" : code;
}

/** Países con España y la UE primero (orden habitual en formularios españoles). */
export function countriesForSelect(): Country[] {
  const spain = byCode.get("ES");
  const eu = COUNTRIES.filter((country) => country.code !== "ES" && EU_MEMBER_CODES.has(country.code));
  const rest = COUNTRIES.filter((country) => !EU_MEMBER_CODES.has(country.code));
  return [...(spain ? [spain] : []), ...eu, ...rest];
}
