              ...
              <article className="overflow-hidden rounded-md border bg-background" key={item.localId}>
                <div className="flex flex-col gap-3 border-b bg-muted/20 px-4 py-3 md:flex-row md:items-center md:justify-between">
                  <div className="flex min-w-0 items-center gap-3">
                    {item.status === "POSTED" ? <CheckCircle className="h-5 w-5 shrink-0 text-success" weight="fill" /> : item.status === "FAILED" ? <XCircle className="h-5 w-5 shrink-0 text-destructive" weight="fill" /> : ["WAITING", "UPLOADING", "PENDING", "PROCESSING", "POSTING"].includes(item.status) ? <SpinnerGap className="h-5 w-5 shrink-0 animate-spin text-primary" /> : <FileText className="h-5 w-5 shrink-0 text-primary" />}
                ...
                {item.status === "DONE" || item.status === "POSTING" || item.status === "POSTED" ? (
                  <div className="space-y-4 p-4">
                    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-6">
                      ...
                      <div className="space-y-2"><Label htmlFor={`currency-${item.localId}`}>Moneda</Label><Input disabled={item.status !== "DONE"} id={`currency-${item.localId}`} maxLength={3} onChange={(event) => patchItem(item.localId, { currencyCode: event.target.value.toUpperCase() })} value={item.currencyCode} />{item.currencyCode !== baseCurrencyCode ? <p className="text-xs text-warning">Requiere conversión a {baseCurrencyCode}</p> : null}</div>
                    </div>
                    ...
                    {item.draft?.warnings.length ? <div className="text-sm text-warning"><p className="flex items-start gap-2"><WarningCircle className="mt-0.5 shrink-0" />{item.draft.warnings.join(" ")}</p>{item.draft.warnings.some((warning) => warning.toLocaleLowerCase().startsWith("bloqueo:")) ? <label className="mt-2 flex items-center gap-2 pl-6"><input checked={item.acknowledgeBlocking} onChange={(event) => patchItem(item.localId, { acknowledgeBlocking: event.target.checked })} type="checkbox" />He corregido y revisado los datos bloqueantes</label> : null}</div> : null}
                    {totalsMismatch ? <p className="text-sm text-destructive">Las líneas suman {formatMoney(total, item.currencyCode)} y el documento indica {formatMoney(item.draft?.totalAmount ?? 0, item.currencyCode)}.</p> : null}
                    {item.duplicate.level !== "none" ? <div className={`rounded-[2px] border px-3 py-2 text-sm ${item.duplicate.level === "exact" ? "border-destructive bg-destructive/10 text-destructive" : "border-warning bg-warning/10 text-warning"}`} role="status"><p className="font-medium">{item.duplicate.level === "exact" ? "Duplicado exacto" : "Coincidencia por fecha e importe"}</p>{item.duplicate.matches.map((match) => <a className="mt-1 block underline" href={`/expenses/${match.invoiceId}`} key={match.invoiceId}>Revisar {match.number}</a>)}{item.duplicate.level === "possible" ? <label className="mt-2 flex items-center gap-2"><input checked={item.acknowledgePossible} onChange={(event) => patchItem(item.localId, { acknowledgePossible: event.target.checked })} type="checkbox" />Confirmo que es una factura distinta</label> : null}</div> : null}
                    {item.error ? <p className="text-sm text-destructive" role="alert">{item.error}</p> : null}
                ...
                ) : item.error ? <div className="flex items-center justify-between gap-3 p-4"><p className="text-sm text-destructive" role="alert">{item.error}</p>{item.jobId ? <Button onClick={() => void retryItem(item)} type="button" variant="outline">Reintentar</Button> : null}</div> : null}
              </article>
            );
          })}
        </div>
