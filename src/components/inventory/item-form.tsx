...
export function ItemForm({ id, initialValues = defaults }: { id?: string; initialValues?: ItemFormValues }) {
  ...
  return <form className="space-y-6" onSubmit={submit}><div className="grid gap-4 md:grid-cols-2"><label className="space-y-1.5 text-sm font-medium">Nombre<Input autoFocus onChange={(event) => set("name", event.target.value)} required value={values.name} /></label><label className="space-y-1.5 text-sm font-medium">SKU<Input className="font-mono" onChange={(event) => set("sku", event.target.value.toUpperCase())} required value={values.sku} /></label><label className="space-y-1.5 text-sm font-medium">Precio de venta<Input min="0" onChange={(event) => set("salePrice", event.target.value)} required step="0.01" type="number" value={values.salePrice} /></label><label className="space-y-1.5 text-sm font-medium">Coste de referencia<Input min="0" onChange={(event) => set("costPrice", event.target.value)} required step="0.01" type="number" value={values.costPrice} /></label><label className="space-y-1.5 text-sm font-medium">Stock mínimo<Input disabled={values.isService} min="0" onChange={(event) => set("minimumStock", event.target.value)} required step="0.001" type="number" value={values.minimumStock} /></label><label className="flex items-center gap-3 self-end rounded-lg border p-3 text-sm font-medium"><input checked={values.isService} onChange={(event) => set("isService", event.target.checked)} type="checkbox" />Es un servicio sin control de stock</label></div><div className="flex justify-end border-t pt-5"><Button disabled={loading} type="submit">{loading ? "Guardando…" : id ? "Guardar cambios" : "Crear artículo"}</Button></div></form>;
}
