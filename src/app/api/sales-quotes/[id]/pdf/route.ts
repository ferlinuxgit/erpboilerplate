...
import { getSalesQuotePdfData } from "@/server/pdf/sales-document-pdf";
...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const data = await getSalesQuotePdfData(actor.context.company.id, (await params).id);
  if (!data) return NextResponse.json({ message: "Presupuesto no encontrado." }, { status: 404 });
