...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [archived] = await db.update(item).set({ isActive: false }).where(and(eq(item.id, id), eq(item.companyId, ctx.company.id))).returning({ id: item.id }); if (!archived) return NextResponse.json({ message: "Artículo no encontrado." }, { status: 404 }); await recordAudit({ tenantId: ctx.tenant.id, companyId: ctx.company.id, actorUserId: session.user.id, action: "item.archive", entityName: "item", entityId: id }); return NextResponse.json({ ok: true });
}
