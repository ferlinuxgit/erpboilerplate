  ...
  return (
    <main className="min-h-screen overflow-hidden bg-background">
      <nav className="mx-auto flex h-20 max-w-[1480px] items-center justify-between px-5 sm:px-8">
        <Link className="flex shrink-0 items-center gap-3 whitespace-nowrap" href="/"><span className="grid size-9 place-items-center rounded-lg bg-primary text-xs font-bold text-primary-foreground">ER</span><span className="font-semibold">ERP Suite</span></Link>
      ...
      <section className="surface-grid border-y">
        <div className="mx-auto grid min-h-[calc(100vh-5rem)] max-w-[1480px] lg:grid-cols-[1.05fr_0.95fr]">
          ...
          <div className="relative flex items-center border-t bg-primary p-5 text-primary-foreground sm:p-8 lg:border-l lg:border-t-0">
            <div className="w-full overflow-hidden rounded-2xl border border-primary-foreground/15 bg-primary-foreground/[0.06] p-5 shadow-2xl shadow-black/10 backdrop-blur sm:p-7">
              ...
              <div className="mt-5 grid gap-px overflow-hidden rounded-xl bg-primary-foreground/15 sm:grid-cols-2">
                {[["Facturación", "€ 84.320"], ["Pendiente de cobro", "€ 12.480"], ["Clientes activos", "128"], ["Alertas de stock", "4"]].map(([label, value]) => <div className="bg-primary p-5" key={label}><p className="text-xs text-primary-foreground/55">{label}</p><p className="mt-4 text-2xl font-semibold">{value}</p></div>)}
              </div>
